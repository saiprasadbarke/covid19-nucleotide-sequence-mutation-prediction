...
def create_merged_data(sequences_file: str, clades_file: str, sequence_clade_merged_file: str):
    description_sequence_dict = {
        sequence_record.description: str(sequence_record.seq)
    ...
    merged_dict = {
        description: tuple(d[description] for d in ds)
        for description in description_sequence_dict
        if description in description_clade_dict
    }
    clade_sequence_merged_dict = {}
    for valid_clade in LIST_OF_CLADES:
        if single_clade_sequences_dict := {
            sequence_clade_tuple[0]: ""
            for sequence_clade_tuple in merged_dict.values()
